...
router.get("/editrecord", (req, res, next) => {
  try {
    ...
    if (user != null) {
      ...
      let endp = `http://localhost:4000/editrecord/${user_id}/${vinyl_id}`;
...
router.post(
  ...
  (req, res) => {
    ...
    let endpoint = `http://localhost:4000/editrecord/${u_id}/${v_id}`;
    axios
...
router.delete("/editrecord", (req, res, next) => {
  try {
    ...
    let u_id = req.body.userIdField;
    let endp = `http://localhost:4000/editrecord/${record_id}`;
    axios
      .delete(endp)
      .then((response) => {
